...
class MemoryManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.project_mem_dir = get_project_mem_dir(project_root)

    def is_initialized(self) -> bool:
        """Check if project memory is initialized (global state file exists)."""
        return get_state_file(self.project_root).exists()
    ...
    def initialize(self, project_name: str, description: str = "") -> None:
        """Initialize the unimem memory structure."""
        logger.info(f"Initializing Unimem project memory for {project_name}")
        ...
        # Create local symlink .unimem pointing to the global directory
        local_link = self.project_root / ".unimem"
        if not local_link.exists():
            try:
                local_link.symlink_to(self.project_mem_dir, target_is_directory=True)
            except Exception as e:
                logger.debug(f"Failed to create symlink: {e}. Falling back to copying rules directly.")
    ...
    def save_state(self, state: ProjectState, update_memory: bool = True) -> None:
        """Save the ProjectState to state.json and optionally update memory.md and rules."""
        ...
        if update_memory:
            self.update_memory_md(state)
            sync_project_rules(self.project_root)
        ...
        for session_file in sessions_dir.glob("session_*.json"):
            try:
                ...
                if age_seconds >= orphan_threshold_seconds:
                    logger.info(f"Recovering orphan session {session.session_id} (tool={session.tool})")
    ...
    def rebuild_state_from_events(self, update_memory: bool = True) -> ProjectState:
        """Rebuild state from recorded events."""
        from unimem.core.summarizer import LocalSummarizer
    ...
    def record_events_batch(self, events: List[Event], auto_snapshot: bool = True) -> List[Path]:
        """Record a batch of events to the events folder."""
        events_dir = get_events_dir(self.project_root)
        events_dir.mkdir(parents=True, exist_ok=True)
    ...
    def archive_old_completed_tasks(self, state: ProjectState) -> None:
        """Prune done tasks/features older than 7 days from state.json."""
        archive_file = self.project_mem_dir / "archive.json"
...
"""
    ...
    def update_memory_md(self, state: ProjectState) -> None:
        """Generate a Markdown memory document representing current project status."""
        memory_file = get_memory_md(self.project_root)
