"""Pydantic schemas representing ProjectState, Event, Session, and FileOperation structures for Unimem v2.0.0."""
...
class ProjectState(BaseModel):
    ...
    tech_stack: List[str] = Field(default_factory=list, description="List of technologies, frameworks, and languages used.")
    architecture: List[str] = Field(default_factory=list, description="Architecture notes or design details.")
    coding_preferences: List[str] = Field(default_factory=list, description="Style guidelines or programming preferences.")
    past_summaries: List[str] = Field(default_factory=list, description="Chronological record of past project summaries.")
    custom_rules: List[str] = Field(default_factory=list, description="User-defined custom rules or instructions.")
